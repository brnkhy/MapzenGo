...
public class dummy : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (this._destination != null)
        {
            ...
            var player = GameObject.Find("ThirdPersonController").gameObject.GetComponent<ThirdPersonCharacter>().transform;
            ////rgd.MovePosition(dest);
            //player.Move(_destination, false, false);
            player.position = Vector3.MoveTowards(player.position, _destination, Time.deltaTime * 100);
        }
    }
    ...
    public void Move()
    {
        ...
        int zoom = 18;
        float[] debuglatlon = Array.ConvertAll(inplatlon.text.Split(','), s => float.Parse(s));

        float lat = debuglatlon[0];
        float lon = debuglatlon[1];
